...
- (void)didSelectDate:(KalDate *)date
{
  ...
  [tableView flashScrollIndicators];
    ...
    [date retain];
    if([self.popController.delegate respondsToSelector:@selector(didSelectDate:)])
        [self.popController.delegate performSelector:@selector(didSelectDate:) withObject:date];
    [date release];
}
...
- (void)loadView
{
  if (!self.title)
    self.title = @"Calendar";
    //Rock
    CGRect frame = CGRectMake(0,0,320,260);
